		...
		<c:choose>
			<c:when test="${fn:length(requestScope.result) == 0}">
				<div style="margin-top: 100px;" align="center">
					<p>Sorry, no matching job found!</p>
				</div>
			</c:when>
			<c:otherwise>
				<div style="margin-top: 100px" align="center">
					...
					<form action="searchjob" method="post">
						...
						<c:if test="${pgbeg > 1}">
							<a class="btn btn-default"
								href="searchjob?key=${requestScope.key}&value=${requestScope.value}&pageId=0">F</a>
						</c:if>
						<c:forEach var="page" begin="${pgbeg}" end="${pgend}" step="1">
							<c:if test="${page == (requestScope.pageId + 1)}">
								<a class="btn btn-primary"
									href="searchjob?key=${requestScope.key}&value=${requestScope.value}&pageId=${page - 1}">${page}</a>
							</c:if>
							<c:if test="${page != (requestScope.pageId + 1)}">
								<a class="btn btn-default"
									href="searchjob?key=${requestScope.key}&value=${requestScope.value}&pageId=${page - 1}">${page}</a>
						...
						<c:if test="${pgend < total}">
							<a class="btn btn-default"
								href="searchjob?key=${requestScope.key}&value=${requestScope.value}&pageId=${total - 1}">L</a>
						</c:if>
					</form>
				</div>
			</c:otherwise>
